#!/bin/bash
# Submit the stage 4 (SDG) array for the "knowledge_list" task. All shared
...
#   bash scripts/submit_nemotron_cc/04A3_submit_sdg_array_knowledge_list.sh
#
#   # one shard (e.g. shard 0 — a smoke test over 1/NUM_SHARDS of the data)
#   ARRAY_RANGE=0 bash scripts/submit_nemotron_cc/04A3_submit_sdg_array_knowledge_list.sh
#
#   # a range of shards (indices 0..9 inclusive)
#   ARRAY_RANGE=0-9 bash scripts/submit_nemotron_cc/04A3_submit_sdg_array_knowledge_list.sh
#
#   # specific shards (e.g. rerun failed ones)
#   ARRAY_RANGE=3,17,42 bash scripts/submit_nemotron_cc/04A3_submit_sdg_array_knowledge_list.sh
#
